...
import { startJourneyService ,endJourneyService,markStoppageService,getJourneyDetailsById} from "../services/journeyService";
...
export async function startJourney(ctx: Context): Promise<void> {
  ...
  try {
    const journey = await startJourneyService(
      busID,
      direction
    );
...
export async function endJourney(ctx: Context): Promise<void> {
    ...
    try {
      ...
      console.log(error);
      ctx.status = 500;
      ctx.body = { error: "An error occurred while marking the journey as complete" };
    }
  }
...
export async function markStoppage(ctx: Context): Promise<any> {
  ...
  try {
    ...
    ctx.status = 200;
    ctx.body = { message: "Stoppage marked successfully", journey };
  } catch (error) {
    console.log(error);
    ctx.status = 500;
